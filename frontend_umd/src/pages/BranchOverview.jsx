...
const Overview = () => {
    ...
    const fetchBranchPerformance = async () => {
        try {
            const res = await API.post('/dashboard/branch-performance', {}, { withCredentials: true });
            const data = res.data.branch_performance;
            if (data.length > 0) {
                setPerformanceData(data[0]);  // manager sees only 1 branch
            } else {
                setError("No branch data found.");
            }
        } catch (err) {
            console.error(err.response?.data || err.message);
            setError(err.response?.data?.error || 'Failed to fetch branch performance.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleViewProfitLoss = async (branchId) => {
        ...
        try {
            const res = await API.get(`/dashboard/reports/profit-loss/summary?year=${filterYear}&month=${filterMonth}`, {
                withCredentials: true
            });
            ...
            {performanceData && (
                <div className="table-responsive mt-4">
                    <table className="table table-bordered table-striped text-center">
                        ...
                        <tbody>
                            <tr>
                                <td>{performanceData.branch_name}</td>
                                <td>Rs.{performanceData.total_budget}</td>
                                <td>Rs.{performanceData.total_expense}</td>
                                <td> {performanceData.remaining_budget < 0 ? "Rs.0" : `Rs.${performanceData.remaining_budget}`}</td>
                                <td>
                                    <span className={`badge ${performanceData.remaining_budget < 0 ? 'bg-danger' : 'bg-success'}`}>
                                        {performanceData.remaining_budget < 0 ? 'Loss' : 'Profit'}
                                    </span>
                                </td>
                                <td> {performanceData.over_budget_amount > 0 ? `-Rs.${performanceData.over_budget_amount}` : "Rs.0"}</td>
                                <td>{performanceData.alerts_count}</td>
                                <td><button className="btn btn-sm me-2" style={{ backgroundColor: '#003153', color: 'white' }} onClick={() => handleViewProfitLoss(performanceData.branch_id)}> View </button></td>
                                <td>
                                    <div className="d-flex justify-content-center">
                                        <button
                                            className="btn btn-sm"
                                            style={{ backgroundColor: '#003153', color: 'white' }}
                                            onClick={() => handleShowChart(performanceData.branch_id, performanceData.branch_name)}
                                        >
                                            Show Chart
                                        </button>
                                    </div>
